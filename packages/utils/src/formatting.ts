 ...
 *
 * `@louez/utils` has no access to the web app's i18n config, so the BCP 47 tag
 * comes from the environment. Falls back to `fr-FR`, which is what every call
 * site used before this was configurable.
 *
 * Set `NEXT_PUBLIC_FORMAT_LOCALE` (or `FORMAT_LOCALE` for non-Next consumers)
 * to match the instance language — otherwise a German deployment renders
 * "jeu. 27 août" next to fully translated labels.
 */
const FORMAT_LOCALE =
