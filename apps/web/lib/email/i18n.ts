 ...
 */
/**
 * Full BCP 47 tag for an email locale. Amounts and dates in a German mail
 * should read German — previously anything that was not French fell back to
 * an English tag.
 */
function localeToTag(locale: EmailLocale): string {
  const land = (localeCountries as Record<string, string>)[locale];
  return land ? `${locale}-${land}` : FORMAT_LOCALE;
}
