...
export async function searchVoiceNumbers(input: {
  country: string;
  /** Digit filter: area code for US/CA, "contains" pattern everywhere else. */
  numberFilter?: string;
  ...
  const numberFilter = input.numberFilter?.trim() || undefined;

  try {
    const numbers = await getVoiceProvider().searchAvailableNumbers({
      country,
      // Twilio only honours AreaCode for NANP numbers; elsewhere the same
      // digits are matched anywhere in the number via Contains.
      areaCode: supportsAreaCodeFilter(country) ? numberFilter : undefined,
      contains: supportsAreaCodeFilter(country) ? undefined : numberFilter,
      limit: 12,
    });
    return { numbers };
  } catch (error) {
    log.error(
      "phone",
      `number search failed: ${error instanceof Error ? error.message : String(error)}`,
    );
...
export async function provisionVoiceNumber(input: {
  ...
  try {
    await db.transaction(async (tx) => {
      ...
      await tx
        ...
        .set({
          ...
          status: "active",
          nextRenewalAt: isNumberRentalEnabled() ? addOneMonthClamped(activatedAt) : null,
    ...
    try {
      ...
      await db.delete(storePhoneNumbers).where(eq(storePhoneNumbers.id, reserved.rowId));
    } catch (releaseError) {
      await db
        .update(storePhoneNumbers)
        .set({
    ...
    const insufficient = error instanceof Error && error.message === "insufficient_rental_credits";
    if (!insufficient) {
      log.error(
        "phone",
    ...
    return {
      error: insufficient ? "errors.insufficientAiCredits" : "errors.provisioningFailed",
    };
  }
...
export async function releaseVoiceNumber(): Promise<{ success: true } | { error: string }> {
  const auth = await authorizeProvisioning();
  if (!auth.ok) return { error: auth.error };
  const { store } = auth;
