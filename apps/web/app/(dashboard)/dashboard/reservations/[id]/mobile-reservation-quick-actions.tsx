...
export const MobileReservationQuickActions = ({
  ...
  const statusAnalyticsAction =
    actions.statusAction === "pickup"
      ...
      : actions.statusAction === "return"
        ...
        : actions.statusAction === "confirm"
          ? reservationAnalyticsActions.acceptRequest
          : null;
  ...
  return createPortal(
    // Same mobile treatment as StepActions and the drawer footers: the
    // actions share the whole bar, on comfortable tap targets ending well
    // above the home indicator.
    <div
      className="fixed inset-x-0 bottom-0 z-40 border-t bg-background/95 px-4 pt-2 pb-[max(calc(env(safe-area-inset-bottom,0px)+--spacing(5)),--spacing(6))] shadow-lg backdrop-blur md:hidden"
    ...
    >
      <div className="mx-auto flex w-full max-w-xl gap-2">
        {actions.showPaymentAction && (
          <Button
            size="lg"
            className="h-12 min-w-0 flex-1 rounded-xl px-4 text-sm font-semibold"
            disabled={disabled}
