      ...
      <Card>
        ...
        <CardContent>
          {codes.length === 0 ? (
            <EmptyState
              icon={TagIcon}
              title={t("noPromoCodes")}
              description={t("noPromoCodesDescription")}
              className="rounded-lg border border-dashed"
              action={<Button onClick={openCreateDialog}>{t("createCode")}</Button>}
            />
          ) : (
                    ...
                    <div className="flex items-center gap-4 min-w-0">
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="mt-1 flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted-foreground">
                          <span>
                            {t("usageLabel")}: {formatUsage(code)}
                          </span>
                          {code.minimumAmount && parseFloat(code.minimumAmount) > 0 && (
                            <span>
                              {t("minLabel")}:{" "}
                              {formatCurrency(parseFloat(code.minimumAmount), currency)}
                            </span>
                          )}
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogClose
              render={<Button variant="destructive" isPending={isLoading} />}
